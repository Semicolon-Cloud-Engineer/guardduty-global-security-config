...
if [ -z "$3" ]; then
  read -p "Enter Lark Webhook URL: " LARK_WEBHOOK
else
  LARK_WEBHOOK=$3
fi
...
if [ -z "$5" ]; then
  read -p "Enter Azure Blob Storage Container Name: " AZURE_CONTAINER_NAME
else
...
# Test Lark Webhook
test_component "Lark Webhook" "curl -s -X POST -H 'Content-type: application/json' --data '{\"text\": \"Testing Lark Webhook\"}' $LARK_WEBHOOK >/dev/null"

# Test Azure Storage connectivity
test_component "Azure Blob Storage" "az storage blob list --account-name $AZURE_STORAGE_ACCOUNT --container-name $AZURE_CONTAINER_NAME --query \"[].name\" >/dev/null"
...
isolate_issue() {
  ...
  if [ -n "$LARK_WEBHOOK" ]; then
    curl -X POST -H 'Content-type: application/json' --data '{"text": "Issue detected and isolated. Logs collected."}' "$LARK_WEBHOOK"
  fi
  ...
  if [ -n "$AZURE_STORAGE_ACCOUNT" ] && [ -n "$AZURE_CONTAINER_NAME" ]; then
    az storage blob upload --account-name $AZURE_STORAGE_ACCOUNT --container-name $AZURE_CONTAINER_NAME --file "$ARCHIVE" --name "$ARCHIVE"
    log_message "Logs uploaded to Azure Blob Storage: $AZURE_STORAGE_ACCOUNT/$AZURE_CONTAINER_NAME/$ARCHIVE"
  fi
...
document_issue() {
  ...
  echo "Logs archived to Azure: $AZURE_STORAGE_ACCOUNT/$AZURE_CONTAINER_NAME/$ARCHIVE" >> "$ISSUE_REPORT"
